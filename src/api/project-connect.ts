...
import {
  ApiJoinUsFormFields,
  JoinUsFormFields,
  ApiDailyCheckAppFormFields,
  DailyCheckAppFormFields,
} from '@/project/types';
...
export const sendDailyCheckAppContactUsFormFx = createRequestFx(
  async (
    formFields: DailyCheckAppFormFields
  ): Promise<ApiDailyCheckAppFormFields | null> => {
    const { firstname, lastname, school_id, email, message } = formFields;
    const result = {
      firstname: firstname,
      lastname: lastname,
      school_id: school_id,
    ...
    return request({
      url: `api/dailycheckapp_contact/dailycheckapp_contact/`,
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
